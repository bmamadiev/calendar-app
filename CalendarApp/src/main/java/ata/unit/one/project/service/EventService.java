...
public class EventService {
    ...
    private final HashMap<String, List<Event>> cache = new HashMap<>();
    ...
    public List<Event> shareCalendarEvents(String ownerPersonId, String personId) {
        ...
        if (response == null) {
            throw new IllegalArgumentException(format("Please check that person ids %s and %s are correct", ownerPersonId, personId));
        }
        List<Event> events = processResponse(response);
